...
@interface FBYScanBankCard ()

@end

@implementation FBYScanBankCard
...
- (void)parseBankImageBuffer:(CVImageBufferRef)imageBuffer {
    ...
    CGRect effectRect = [FBYDesign getEffectImageRect:size];
    CGRect rect = [FBYDesign getGuideFrame:effectRect];
    ...
    if(resultLen > 0) {
        ...
        int charCount = [FBYDesign docode:result len:resultLen];
        if(charCount > 0) {
            CGRect subRect = [FBYDesign getCorpCardRect:width height:height guideRect:rect charCount:charCount];
            self.isHasResult = YES;
            UIImage *image = [UIImage getImageStream:imageBuffer];
            __block UIImage *subImg = [UIImage getSubImage:subRect inImage:image];
            
            char *numbers = [FBYDesign getNumbers];
            ...
            FBYBankCardModel *model = [FBYBankCardModel new];
